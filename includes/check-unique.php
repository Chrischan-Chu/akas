...
try {
  ...
  // Normalize numbers (digits only)
  $digitsOnly = static function (string $v): string {
    return preg_replace('/\D+/', '', $v);
  };

  // Build a UNION existence query to check uniqueness across relevant tables.
  // Note: DB UNIQUE indexes do NOT enforce uniqueness across different tables.
  if ($type === 'email' || $type === 'clinic_email') {
  ...
  } elseif ($type === 'phone' || $type === 'clinic_contact') {
